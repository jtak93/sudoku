...
export function SudokuCreate(maxNum) {
    ...
    function placeNumber(num, arr) {
        ...
        const horizontalBoxesUsed = []; //records which sub box has been used, in the current sub box row
    ...
    for (let i = numSet.length - 1; i >= 0; --i) {
        const workingArray = [];//holds the rows we are currently working with and/or have already wored with
